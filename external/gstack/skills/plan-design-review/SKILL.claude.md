...
_UPDATE_CHECK=$(${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-config get update_check 2>/dev/null || echo "true")
...
Before each AskUserQuestion, choose `question_id` from `${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/scripts/question-registry.ts` or `{skill}-{slug}`, then run `printf '%s' "<question summary>" | ${GSTACK_RUNTIME_ROOT:-$HOME/.gstack/runtime}/bin/gstack-question-preference --check "<id>" --summary-stdin` (piped summary feeds the one-way keyword net, #2024). `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
